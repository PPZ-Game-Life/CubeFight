...
### 4.3.1 确认性与无效点击反馈（2026-05-03）
- **合并 / 吞噬确认性**: 主动块沿轻抛物线移动，目标块保留轻压缩 / 回弹节奏；吞噬动作额外保留“被吸入”的目标残影与落点冲击环，优先让玩家确认“我点到了、系统执行了”。
- **无效点击反馈**: 对非合法目标、错误切片、无效炸弹目标统一做“短暂抖动 + 红色脉冲环 + 轻提示高亮”反馈，不再只靠状态文案。
- **实现边界**: 无效反馈使用一次性 transient state + 轻量几何体，不引入粒子、镜头震动或重后处理，避免移动端和浏览器外放场景过度嘈杂。
...
### 11.32 相机全俯仰开放与点击反馈增强（2026-04-11）
- **相机轨道调整**：`CameraRig` 当前使用 `TrackballControls` 承担任意 3D 旋转，允许 `X / Y / Z` 三轴自由翻转与滚动，满足完整空间观察需求；`gameStore` 继续只记录由相机位置反推的 `yaw / pitch` 供切片映射使用，不额外约束相机 roll。
- **回正操作**：支持 `双击屏幕回正`，会以约 `280ms` 的缓动动画把相机目标点恢复到棋盘中心，并重置为默认正视朝向与世界 `up=(0,1,0)`，避免硬切镜头带来的突兀感。
- **点击反馈增强**：`CubeMesh` 点击后新增即时选中音效、短时外圈白色脉冲、rim 强化与轻微缩放爆点；同时常态 `selected / highlighted` 的缩放、环形脉冲与发光强度整体上调，避免玩家觉得“点到了但没反馈”。
- **交互优先级修正**：方块 mesh 在 `pointerdown` 阶段就会提前 `stopPropagation()`，避免自由旋转相机在教程/局内点击时抢走事件，导致“看起来点中引导目标但没有选中/执行”。
- **触摸端点击兜底**：方块交互已改为 `pointerup` 主触发、`click` 兜底，并对短时间重复触发做去重。这样在手机触摸场景下，即使原生 `click` 被控件链或浏览器手势吞掉，教程与局内选中仍可正常命中。
...
### 11.34 局外 / 局内背景底色统一（2026-05-03）
- **问题背景**：从大厅切入单局时，浏览器默认渐变背景会在 R3F 首帧落地前短暂露出，形成一段偏紫的闪色过渡，看起来像卡顿。
- **当前策略**：`index.html` 的 `body` 与 `#app` 背景统一收口为与游戏渲染一致的深色底色，`GameCanvas` 也显式设置相同基础色，确保菜单 -> 单局 -> 返回大厅的过渡期间没有突兀底图跳变。
- **实现原则**：过渡态优先保证“底色一致”，不在切场时依赖任意渐变或默认浏览器背景色兜底。

### 11.35 新手引导文案轻量化（2026-05-03）
- **目标**：降低首局阅读压力，教学提示从解释型长句收口为“短句 + 立即操作”。
- **当前策略**：第 1 关 7 步结构不变，只压缩中英文本；完成面板改为祝贺语，并把下一步明确导向“无尽模式挑战更高分”。
- **实现边界**：不改教学步数、不改棋盘配置、不改成功判定与 `900ms` 结算观察窗口。
- 请 `@主策划 樊老师` 同步检查 `Doc/GameDesign/` 中新手教学文案与完成后目标口径，确保策划稿也从“理解规则”转成“完成教学后追更高分”。

### 11.36 设置页规则与音量入口（2026-05-03）
- **设置规则入口**：`MainMenu` 设置弹窗新增“游戏规则”入口，规则内容由 `GameRulesDialog` 统一承载，避免大厅和局内各写一套文案。
- **局内规则入口**：`HUD` 左下角工具区新增 `?` 规则按钮，打开同一套规则弹窗；弹窗只占 UI 层，不改 `gameStore` 的运行状态，避免规则查看触发暂停/重开等副作用。
