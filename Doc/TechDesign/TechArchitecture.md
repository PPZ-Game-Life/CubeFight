...
- **菜单展示盘面约束**：主菜单背景演示盘面固定从 `3x3x3` authored 关卡中选取，不再混入 4x4x4 / 5x5x5，确保初见观感与新手期认知一致。
...
- **教学拆步**：第 1 关不再连续硬切操作，而是拆成 `说明步(info) -> 操作步(action)` 的双拍结构。玩家先读文字，再点 `继续` 进入真实操作。
...
- 请 `@主策划 樊老师` 将第 1 关教学流程同步更新到 `Doc/GameDesign/`，特别说明“先解释蓝/红/黄规则，再进入对应操作步骤”、“成功动作后保留约 900ms 观察窗口”，以及“教学步中不展示失败弹层”的节奏，避免策划稿与实机教学手感脱节。
